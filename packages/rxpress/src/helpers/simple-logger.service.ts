...
class SimpleLogger implements RxpressLogger {
    ...
    private write(level: LogPayload['level'], message: string, meta?: Record<string, unknown>) {        
        const consoleTarget = level === 'error' ? console.error : level === 'warn' ? console.warn : console.log;
        consoleTarget(`[${level.toUpperCase()}] ${message}`, meta && Object.keys(meta).length ? meta : '');
    }
}
